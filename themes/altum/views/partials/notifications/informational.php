...
<?php ob_start() ?>
<div role="dialog" class="altumcode-wrapper altumcode-wrapper-<?= $notification->settings->border_radius ?> <?= $notification->settings->shadow ? 'altumcode-wrapper-shadow' : null ?> <?= ($notification->settings->direction ?? 'ltr') == 'rtl' ? 'altumcode-rtl' : null ?> altumcode-informational-wrapper" style='font-family: <?= $notification->settings->font ?? 'inherit' ?>!important;background-color: <?= $notification->settings->background_color ?>;border-width: <?= $notification->settings->border_width ?>px;border-color: <?= $notification->settings->border_color ?>;<?= $notification->settings->background_pattern_svg ? 'background-image: url("' . $notification->settings->background_pattern_svg . '")' : null ?>;'>
    <div class="altumcode-informational-content">
        <?php if(!empty($notification->settings->image)): ?>
        <img src="<?= (substr($notification->settings->image, 0, 4) === 'http') ? $notification->settings->image : \Altum\Uploads::get_full_url('notifications') . $notification->settings->image ?>" class="altumcode-informational-image" alt="<?= $notification->settings->image_alt ?>" loading="lazy" />
        <?php endif ?>
        ...
        <div>
            ...
            <?php if($notification->settings->display_branding): ?>
                ...
                <?php else: ?>
                    <a href="https://eticaret.pro/" class="probrand2"></a> <?php endif ?>
            <?php endif ?>
        </div>
    </div>
</div>
